...
export default function EnhancedHealthSystem({
 ...
 <div className={`relative bg-gray-800 h-4 overflow-hidden border border-gray-600 ${getHealthBarGlow()}`}>
 {/* Health Bar Fill */}
 <motion.div
 ...
 className="text-xs bg-red-600 text-white px-2 py-0.5 font-bold"
 >
 CRITICAL
 </motion.div>
 )}
 ...
 exit={{ opacity: 0, y: -10 }}
 className="text-xs bg-yellow-600 text-white px-2 py-0.5 font-bold"
 >
 STUNNED
 </motion.div>
 )}
 ...
 {isLow && isActive && (
 <motion.div
 className="absolute -top-1 -left-1 -right-1 -bottom-1 border-2 border-yellow-500/50 pointer-events-none"
 animate={{
 opacity: [0.5, 1, 0.5],
 ...
 <motion.div
 className="absolute -inset-2 bg-green-500/20 pointer-events-none"
 animate={{
 opacity: [0.5, 0],
 scale: [1, 1.1]
 }}
